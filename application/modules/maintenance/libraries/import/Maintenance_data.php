...
class Maintenance_data extends MX_Controller{
    ...
    public function process($transaction_details = []){
        ...
        $sales_orders_columns = $this->maintenance_model->field_list(); // Get bed data colums/fields
        $a = [
            array_search('id',array_column($sales_orders_columns,'Field')),
            array_search('date_uploaded',array_column($sales_orders_columns,'Field')),
        ];
        foreach($a AS $a){
        ...
        $this->db->trans_begin();
        $this->maintenance_model->batch_insert_data($aData);
