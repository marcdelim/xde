...
class App extends MX_Controller
{
   ...
   public function __construct()
   {
      ...
      $ainfo['module_name'] = $this->environment->module_name;
      $ainfo['user_id'] = $this->session->userdata('user_id');
      $ainfo['user_permission'] = $this->session->userdata('permission');
      $this->load->vars($ainfo);
	  
	  $this->rest->initialize(array('server' => site_url('api')));
   ...
   public function footer($sfile_override = "",$aheader_info = array())
   {
	  ...
	  // $this->ajs_source[0] = array("sfile" => $this->environment->assets_path . 'core/js/core.js', "cache" => false);
   ...
   public function content($sfile,$ainfo = array())
   {
	   ...
	   return;
      $data = $this->load->view($sfile,$ainfo,true);
   ...
   {   
      ...
      if( filter_var ( $sfile , FILTER_VALIDATE_URL ) ) {      
         ...
         if ( $_SERVER['HTTP_HOST'] === $aurl['host'] ) {         
            $this->acss_source[] = array('sfile' => $sfile,"cache" => false,"attributes" => $aattributes);         
         } else {            
            $this->acss_source[] = array('sfile' => $sfile,"cache" => $bcache,"attributes" => $aattributes);         
         }         
      } else {      
         $apath_info = pathinfo($sfile);         
         if ( $apath_info['dirname'] ==='.' ) {         
   ...
   {  
      ...
      if ( filter_var ( $sfile , FILTER_VALIDATE_URL ) ) {
         ...
         $apath_info = pathinfo($sfile);      
         $this->ajs_source[self::$ijs_counter++] = array("sfile" => $this->_get_file_path($sfile,$sdir) . '.js',"cache" => $bcache,"attributes" => $aattributes);              
      }     
   }
